...
for no_of_agents in range(15,28):
    for instances in range(1,4):
        ...
        file_name_1 = file_name[0:1] + "_" + "sequential" + ".csv"  #creating a csv file for each probabilty distribution
        myresult = list()  #list to store the results              
        ...
        if __name__=="__main__":
            #maxiterations = int(input("Enter no of iterations : "))
            maxiterations = 10
            start_time = timeit.default_timer()
            tabuList = []
            CS = random_cs_generator()
            #print("Initial random coalition structure : ",CS)
            currentBest = copy.deepcopy(CS)
            for i in range(maxiterations):
                cs = copy.deepcopy(CS)
                #print(" Random Coalition structure generated : ",cs,"\n")
